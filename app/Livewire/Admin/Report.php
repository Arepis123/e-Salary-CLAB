...
class Report extends Component
{
    ...
    public function downloadAllReceipts()
    {
        ...
        return $this->downloadReceipts(collect($this->taxInvoices)->pluck('id')->toArray());
    }
    ...
    public function downloadSelectedReceipts()
    {
        ...
        return $this->downloadReceipts($this->selectedInvoices);
    }
    ...
    protected function downloadReceipts($invoiceIds)
    {
        ...
        if ($invoices->isEmpty()) {
            Flux::toast(variant: 'error', text: 'No receipts found.');
        ...
        $zip = new \ZipArchive;
        $zipFilename = 'Tax-Invoices-'.\Carbon\Carbon::create($this->selectedYear, $this->selectedMonth)->format('Y_m').'_'.now()->format('YmdHis').'.zip';
        ...
        Flux::toast(variant: 'error', text: 'Failed to create ZIP file.');
    }
